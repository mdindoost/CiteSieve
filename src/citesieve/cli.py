...
def main():
    ...
    p.add_argument("--title", help="Target paper title (exact match after normalization).")
    p.add_argument("--openalex", action="append",
                   help="OpenAlex W-ID core or with W (e.g., 2916106175 or W2916106175). Can repeat.")
    ...
    if args.openalex:
        for oid in args.openalex:
            ...
            works.append({
                "id_core": core,
                "id_full": f"https://openalex.org/W{core}",
                "title": "(provided)",
                "year": None,
                "cited_by_count": None
            })
    ...
    # Fetch Semantic Scholar citers (strict title match to avoid variants)
    s2_norm = []
    if args.title:
        s2_cands = s2_find_paper_ids(args.title, strict=True)  # strict exact-match (normalized)
        if s2_cands:
            print("\n[S2 PAPER IDS]")
            for pid, ttl, yr in s2_cands:
                print(f"- {pid} | year={yr} | {ttl}")
            for pid, _, _ in s2_cands:
                for cp in s2_fetch_citers(pid, args.year_min, args.year_max):
                    s2_norm.append(normalize_s2(cp))
